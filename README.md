...
7. **Set Up TypeScript for Jest**:
   ...
   >
   > [TypeScript Object-Oriented Programming](002-typescript-oop.md) uses `npm install --save-dev ts-jest` instead of `@jest/globals`.

   - **`babel.config.json`** (same for every chapter):
   ...
   - In the `tsconfig.json` generated by step 6, the fields that typically need adjusting per chapter: `"module"`, `"target"`, and `"types": ["jest"]` — check each chapter's own code samples for the exact values it expects.
...
8. **Follow Along Per Chapter**:
   ...
   > Each chapter is a **separate** project — there is no single root `package.json`. Repeat steps 3–7 inside each chapter's own project folder ([001](001-typescript-basics.md), [002](002-typescript-oop.md), [003](003-typescript-generic.md), [004](004-typescript-validation.md)). [005](005-typescript-study-case.md) documents its own project setup entirely, separate from these steps.
